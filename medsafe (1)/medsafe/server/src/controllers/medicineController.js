...
// GET /api/medicines
exports.listMedicines = async (_req, res, next) => {
  try {
    const items = await Medicine.find().sort({ expiryDate: 1 });
    res.json(items);
  } catch (err) {
    next(err);
  }
...
exports.updateMedicine = async (req, res, next) => {
  try {
    ...
    const doc = await Medicine.findByIdAndUpdate(req.params.id, update, {
      new: true,
    });
    if (!doc) return res.status(404).json({ error: "Not found" });
    scheduleReminderRefresh();
